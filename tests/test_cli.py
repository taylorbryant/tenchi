...
def test_new_scaffolds_a_working_app(
    ...
    assert "--change-plan" in (root / "AGENTS.md").read_text()
    ...
    assert app_map["schema_version"] == 10
    ...
    assert preflight_result["schema_version"] == 10
    ...
    assert listed_evaluations["schema_version"] == 10
    ...
    assert evaluation_report["schema_version"] == 10
...
def test_jobs_cli_writes_checks_and_classifies_snapshots(tmp_path: Path) -> None:
    ...
    assert listed_result["schema_version"] == 10
...
def test_task_cli_lists_runs_and_reports_validation_as_versioned_json(
    ...
    assert listing["schema_version"] == 10
...
def test_verify_produces_one_receipt_against_an_immutable_baseline(
    ...
    assert receipt["source"]["head_commit"] == commit
    assert receipt["source"]["tree_digest"].startswith("sha256:")
    assert len(receipt["source"]["tree_digest"]) == 71
    ...
    assert receipt["errors"] == [
        {
            "stage": "source",
            "message": (
                "source changed while verification was running; rerun verify "
...
def test_openapi_diff_ref_reads_the_snapshot_from_git(
    ...
    assert compatible["schema_version"] == 10
...
def test_make_dry_run_and_json_share_a_versioned_result(
    ...
    assert planned["schema_version"] == 10
...
def test_make_use_case_from_contract_generates_exact_incomplete_boundary(
    ...
    _git(root, "config", "user.name", "Tenchi Tests")
    _git(root, "add", ".")
...
""",
    ...
    _assert_operation_error(
        baseline_without_plan,
        operation="make",
        code="TENCHI_CLI_INVALID_ARGUMENTS",
    )

    planned_change = _tenchi(
        root,
        "make",
        "use-case",
        "projects",
        ...
        target,
        "--plan",
        ".tenchi/changes/create-project.json",
        "--base-ref",
        "HEAD",
        "--dry-run",
        "--json",
    )
    assert planned_change.returncode == 0, planned_change.stdout + planned_change.stderr
    planned_payload = json.loads(planned_change.stdout)
    assert planned_payload["change_plan_path"] == (
        ".tenchi/changes/create-project.json"
    )
    assert planned_payload["change_plan"]["schema_version"] == 1
    assert planned_payload["change_plan"]["baseline"] == {
        "ref": "HEAD",
        "commit": _git(root, "rev-parse", "HEAD").stdout.strip(),
    }
    assert planned_payload["change_plan"]["plan_id"].startswith("sha256:")
    assert not (root / ".tenchi/changes/create-project.json").exists()
    assert not (feature / "use_cases/create_project.py").exists()
    ...
    change_plan_path = root / ".tenchi/changes/create-project.json"
    assert (
        json.loads(change_plan_path.read_text(encoding="utf-8"))
        == json.loads(created.stdout)["change_plan"]
    )
...
def test_contract_change_plan_verifies_a_completed_generated_use_case(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(tmp_path)
    ...
    contracts_path = root / "app/features/todos/contracts.py"
    contracts_path.write_text(
        contracts_path.read_text(encoding="utf-8")
        + """\

verify_todo_contract = contract(
    method="POST",
    path="/todos/verified",
    request=CreateTodo,
    response=Todo,
...
""",
    ...
    generated = _tenchi(
        ...
        "use-case",
        "todos",
        "create_todo_verified",
        "--from-contract",
        "app.features.todos.contracts:verify_todo_contract",
        "--plan",
        ".tenchi/changes/create-todo-verified.json",
        "--base-ref",
        "HEAD",
        "--json",
    )
    ...
    feature = root / "app/features/todos"
    (feature / "use_cases/create_todo_verified.py").write_text(
        """\
...
async def create_todo_verified(
    request: CreateTodo,
    context: AppContext,
) -> Todo:
...
""",
    ...
    (feature / "tests/test_create_todo_verified.py").write_text(
        """\
from app.features.todos.schemas import CreateTodo
from app.infra.memory_todo_repository import MemoryTodoRepository
from app.server.context import AppContext

from ..use_cases.create_todo_verified import create_todo_verified


async def test_create_todo_verified_persists_through_the_repository_port() -> None:
    context = AppContext(todos=MemoryTodoRepository())

    todo = await create_todo_verified(CreateTodo(title="Verify me"), context)

    assert todo.title == "Verify me"
""",
        encoding="utf-8",
    )
    routes_path = feature / "routes.py"
    routes_path.write_text(
        routes_path.read_text(encoding="utf-8")
        .replace(
        ...
        .replace(
            ...
            "    route(list_todos_contract, list_todos),\n"
            "    route(verify_todo_contract, create_todo_verified),\n",
        ),
        encoding="utf-8",
    )
    openapi = _tenchi(root, "openapi", "--write", "openapi.json")
    assert openapi.returncode == 0, openapi.stdout + openapi.stderr
    ...
    assert receipt["ok"] is True
    assert receipt["change_plan"]["ok"] is True
    assert all(check["ok"] for check in receipt["change_plan"]["checks"])
