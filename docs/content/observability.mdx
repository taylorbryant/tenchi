...
Pass the observer to `create_app(use_case_observers=...)` for HTTP and
`execute(use_case_observers=...)` for workers, schedules, scripts, and direct
calls. It runs after the surrounding context scope closes, so transactional
cleanup finishes before telemetry is exported. Input values, return values,
and exception objects are never included.

Keep use-case observers fast too. Both HTTP and `execute()` wait for the
...
| Use-case status and duration across HTTP and `execute()` | `UseCaseOutcome` observer |
