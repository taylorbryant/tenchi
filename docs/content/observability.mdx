...
# Observe behavior without exposing payloads
...
The accepted request ID is safe to use as a header, but may be caller-supplied.
Have the trusted edge replace it if your telemetry requires an edge-generated
identifier. Treat trace headers according to the same deployment policy.

When HTTP enqueues work, copy correlation allowed by that policy into
...
- OpenAPI, job-message, application-tool, or evaluation-policy compatibility gate failures
  before deployment.
