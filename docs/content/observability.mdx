...
an operational task, or `execute()`:

| Field | Meaning |
| --- | --- |
| `use_case` | The invoked async function |
| `entrypoint` | `http`, `execute`, or `task` |
...
| `status` | `succeeded`, `app_error`, `unexpected_response`, `transport_error`, `failed`, or `cancelled` |
| `status_code` | The HTTP status when a response arrived; otherwise `None` |
| `duration_seconds` | Local preparation, transport, and response-validation time |
...
and attach its attempt and queue measurements around `execute()`. The shared
...
| Use-case status and duration across HTTP, tasks, and `execute()` | `UseCaseOutcome` observer |
...
| Worker attempts, queue latency, and correlation | Instrument the consumer around `execute()` |
