...
`tenchi check`, and `tenchi verify` treat an absent default module as not
configured.
...
- Modules directly under `app/` outside `features`, `shared`, `infra`, and
  `server` are rejected.
- Symlinked application paths are rejected so results do not vary with
  Python's directory-walking behavior.
