...
<Callout title="Types without runtime coupling">
  ...
  without importing server composition at runtime. Route binding deliberately
  ignores this app-owned context type while still checking every contract-owned
  boundary annotation. `tenchi map` reads the guarded import from source and
  records the context dependency without adding runtime coupling.
</Callout>
