...
<Callout type="warning" title="Use Streamable HTTP, not legacy SSE">
  Application MCP supports the SDK's Streamable HTTP transport and rejects its
  inherited `sse_app()` and `run_sse_async()` entrypoints. Legacy SSE cannot
  carry Tenchi's independently authenticated request-header scope without
  weakening the identity and visibility guarantees described on this page.
</Callout>
...
Authentication or visibility failures during discovery become one generic
`Tool discovery failed.` MCP error; callback exception messages and the failed
stage are not returned to the client.
