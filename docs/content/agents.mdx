...
integration before interpreting its results. See [upgrade guidance](/stability)
...
See [verifying a generated change](/change-plans#verify-the-requested-structure)
for the complete plan workflow.
...
Failed check steps include bounded output from the project's validation
commands, and verification includes the same output. Review it before sharing
reports: Tenchi does not remove sensitive values printed by tests or scripts.
