...
architecture diagnostics, project checks, and OpenAPI and application-tool
snapshots. Modules such as `tenchi.cli`, `tenchi.doctor`, `tenchi.scaffold`,
and `tenchi.snapshots` implement that command surface; they are not supported
application imports.
