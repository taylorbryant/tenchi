...
If the selected ref genuinely predates the application's first
`evaluations.json`, pass `--allow-missing-evaluation-baseline` once and confirm
the evaluation result includes an `evaluation manifest baseline` metadata
change. Missing evaluation snapshots otherwise fail closed, so a renamed or
mistyped path cannot silently replace the historical policy.
