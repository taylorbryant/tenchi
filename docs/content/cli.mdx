...
operational tasks, use cases, policies, ports, adapters, context types,
...
command in `.mcp.json`.
`--routes`, `--api-routes`, `--preflight`, `--tasks`, and `--snapshot` override the
conventions captured by the server when it starts. `--title`, `--version`,
