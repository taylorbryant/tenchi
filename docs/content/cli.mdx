...
command in `.mcp.json`.
`--routes`, `--api-routes`, and `--snapshot` override the conventions captured
by the server when it starts. `--title`, `--version`, `--description`, and
`--security` override discovered OpenAPI metadata for the diff and check tools.
