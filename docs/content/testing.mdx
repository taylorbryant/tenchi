...
When the application adds durable jobs, application tools, or evaluations,
add the matching test so their snapshots stay exact too:
...
The job and evaluation variants call `["jobs", "--check", "jobs.json"]` and
`["eval", "snapshot", "--check", "evaluations.json"]`. `tenchi new --full`
generates all three. Run the matching `--diff` command before replacing any of
these snapshots: the exact test proves reproducibility, and the historical
diff proves compatibility. See [background jobs](/jobs), [application
tools](/tools), and [AI evaluations](/evaluations) for each workflow.
