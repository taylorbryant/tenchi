...
Run `tenchi jobs --diff jobs.json` before replacing the snapshot. The report
proves that a new consumer can still read messages accepted under the
historical manifest. See [Background jobs](/jobs) for the complete
...
Keep the evaluation policy, which contains no case inputs, reproducible in the
same way:
