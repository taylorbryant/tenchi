...
`retry_on`. Tenchi uses exponential backoff with bounded jitter. A valid
`Retry-After` decimal-integer delay or HTTP date carried by the declared error
can increase the wait, but never beyond `max_delay_seconds`. Malformed and past
