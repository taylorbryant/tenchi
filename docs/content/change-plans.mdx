...
Tenchi resolves `origin/main` to a baseline commit before writing anything.
...
Copy the full commit from `baseline.commit` in the plan into `PLAN_BASE_SHA`:

```shell
PLAN_BASE_SHA="<commit from the plan's baseline.commit>"
uv run tenchi verify \
  --base-ref "$PLAN_BASE_SHA" \
  --change-plan .tenchi/changes/create-project.json \
  --json
```

Use that recorded commit even if `origin/main` has advanced since generation.
Repeating the ref name works only while it still resolves to the plan's commit.
...
in-process evidence as tamper-proof. When the code producer is untrusted, keep
acceptance tests and the process that evaluates them outside the worktree the
producer can edit.
...
- **Baseline.** The Git commit that `--base-ref` resolves to. A ref such as
  `origin/main` can move later, so the receipt records the commit and every
  comparison uses that same historical state.
- **Receipt.** The result of `tenchi verify`: one JSON object covering the
  source digest, verification policy, checks, architecture, compatibility
  reports, and any change plan. `tenchi check` produces a result; only `verify`
  produces a receipt.
- **Evidence.** What a `[verify]` stage in `tenchi.toml` requires, such as a
  passing check or a compatible OpenAPI report. Each stage ends as `passed`,
  `failed`, `skipped`, `not_configured`, or `not_verifiable`.
- **Source digest.** A SHA-256 hash of the application tree, including
  nonignored untracked files, captured before verification and rechecked after
  every project-owned stage.
- **Plan ID.** The `sha256:…` identity of a change plan, derived from its
  content. Any change to the plan produces a new ID.
- **Payload-safe outcomes.** Runtime observer outcomes and evaluation reports
  omit invocation inputs, generated outputs, and exception messages. Manifests
  publish application-supplied schemas, descriptions, and other declaration
  metadata; keep that metadata safe to share. Failed `check` steps retain
  bounded command output, which is also included in `verify` reports. Tenchi
  does not redact what your tests or validation commands print.
- **Compatible.** A change preserves existing callers or the guarantees a
  policy protects. Current inputs must accept historical valid inputs;
  current outputs must remain understandable to existing consumers. Job
  comparisons protect stored messages, while evaluation comparisons protect
  the strength of the quality gate. These comparisons do not require the
