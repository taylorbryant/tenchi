...
successfully, not business correctness. It does not prove that the test called
the use case, that its assertions are strong enough, or that the implementation
satisfies the product requirement. Review the use case, policy, ports, and test
behavior as usual; `tenchi check` and the historical compatibility stages
remain part of the same final receipt.
