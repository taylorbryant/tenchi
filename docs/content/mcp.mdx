...
compatibility, deployment preflight, operational-task discovery, and the
...
unsafe paths are MCP errors. Tenchi snapshots every tool's input and output
schema in CI; breaking or unknown changes require a new protocol version.
...
The map loads registered background jobs through `--jobs`.
