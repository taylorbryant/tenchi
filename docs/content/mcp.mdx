...
diagnostics, generator previews, OpenAPI, tool, and evaluation-policy
...
inside that root, and OpenAPI, application-tool, and evaluation-policy snapshot
...
| `verify` | One completion receipt containing checks, strict architecture evidence, and all three compatibility reports against a required Git ref |
| `check` | Ruff format, Ruff lint, Pyright, pytest, doctor, and the OpenAPI, tool, and evaluation-policy snapshot checks |
...
finding, failed check, generator conflict, or incompatible API, tool, or
...
8. Call `evaluation_diff` before accepting a changed evaluation-policy snapshot.
9. Call `verify` with the pull request base, previous push, or previous release
...
selected ref already contains the OpenAPI and tool snapshots. The returned
