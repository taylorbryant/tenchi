...
# Run validated operational tasks
...
echoed into the result. Direct `TaskRunner.run()` callers receive a
payload-safe `TaskInputError` for invalid input; its `issues` expose only stable
validation kinds.
...
`TaskRunner` does not impose an execution timeout. Let the operator environment
set an outer process deadline, and put an [entrypoint-neutral application
deadline](/execution#set-an-entrypoint-neutral-deadline) inside behavior that
must remain bounded when called through HTTP, tools, jobs, and tasks alike.
